...
//C# TO JAVA CONVERTER WARNING: Java does not have a 'ignoreCase' parameter for the static 'valueOf' method of enum types:
			...
			java.util.ArrayList<Guid> waitList;
			synchronized (_syncRoot)
			{
				waitList = (java.util.ArrayList<Guid>)this._cache[key];
				if (waitList == null)
				{
//C# TO JAVA CONVERTER TODO TASK: There is no Java equivalent to LINQ queries:
					waitList = (from id in parameter.getValues().GetValueOrDefault("jobs", "").split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries) select new Guid(id)).ToList();
				}
			...
			if (waitList.size() > 0)
			{
				...
				{
					java.util.ArrayList<Guid> temp = new java.util.ArrayList<Guid>(waitList);
