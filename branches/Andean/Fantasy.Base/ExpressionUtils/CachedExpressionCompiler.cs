                    ...
                    if (expression != null)
                    {
                        if (func2 == null)
                        {
                            func2 = delegate (MemberInfo _) {
                                ...
                                UnaryExpression expression2 = Expression.Convert(expression3 = Expression.Parameter(typeof(object), "capturedLocal"), memberExpr.Member.DeclaringType);
                                return Expression.Lambda<Func<object, TOut>>(memberExpr.Update(expression2), new ParameterExpression[] { expression3 }).Compile();
                            };
                        }
