		...
		for (Object o : this._list) {
			...
			if (o instanceof IService) {
				...
				try {
					...
					Log.SafeLogError(logger, Resources.getServicesCategory(),
							error,
							Resources.getServiceFailedInitializeMessage(),
							o.getClass());
					throw error;
				}

				if (logger != null) {
					logger.LogMessage(Resources.getServicesCategory(),
							Resources.getServiceInitializedMessage(), o
									.getClass().getName());
				}
			}
		}
	...
	public void uninitializeServices() throws Exception {
		...
		for (int i = this._list.size() - 1; i >= 0; i--) {
			...
			if (o instanceof IService) {
				if (logger != null) {
					logger.LogMessage(Resources.getServicesCategory(),
							Resources.getServiceUninitializedMessage(), o
									.getClass().getName());
				}
