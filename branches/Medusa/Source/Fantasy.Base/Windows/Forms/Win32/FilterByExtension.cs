...
namespace Fantasy.Windows.Forms.Win32
{
	...
	public class FilterByExtension : IFolderFilter
	{
			...
			IntPtr pidlItem)		// The item's PIDL.
		{
			...
			isf.GetDisplayNameOf(pidlItem,(uint)SHGNO.SHGDN_NORMAL | (uint)SHGNO.SHGDN_FORPARSING,out ptrDisplayName);
			...
			aPidl[0] = pidlItem;
			uint Attrib;
			Attrib = (uint)SFGAO.SFGAO_FOLDER;

			int temp;
			temp = isf.GetAttributesOf(1,aPidl,ref Attrib);

			// if item is a folder, accept
			if ((Attrib & (uint)SFGAO.SFGAO_FOLDER) == (uint)SFGAO.SFGAO_FOLDER)
				return 0;
		...
		{
			pgrfFlags = (uint)SHCONTF.SHCONTF_FOLDERS | (uint)SHCONTF.SHCONTF_NONFOLDERS;
			return 0;
		}
